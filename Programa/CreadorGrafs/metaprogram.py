...
data = pd.read_csv('CreadorGrafs/vertexs2.csv', names=['LATITUD', 'LONGITUD', 'SI'], sep=",", comment="#")
graph_data = tuple(zip(data['LATITUD'].values, data['LONGITUD'].values, data['SI'].values))


f = open("CreadorGrafs/test.py", "w")
"""
i = 1
for d in graph_data:
    x,y,z = d
...
for d in graph_data:
    ...
    f.write(f"\t\tself.firstBtn = self.g_val.index(({x},{y}))+1\n")
    ...
    f.write("\t\t\tf = open('CreadorGrafs/arestes2.csv', 'a')\n")
